...
origInds = find(cellfun('isempty',regexp({f(:).name},'2\..*$')));


% Make list of the student name strings that Canvas puts at the beginning
% of every file name.

personStrCell = cell(1,numel(f)); % empty cell array

for pp = 1:numel(f) % "pp" for "person"
    
    % Every file name downloaded from Canvas begins with "[student_name]_"
    personStrEndInd = regexp(f(pp).name,'^[a-z]*','end');
    personStrCell{pp} = f(pp).name(1:personStrEndInd);
...
for ii = 1:numel(uniqueStrCell)
    ...
    imOrig = imread([INPUT_DIR f(thisOrigInd).name]);
    ...
    if ~isempty(thisMarkedInd)
        thisMarkedImFN = [INPUT_DIR f(thisMarkedInd).name];
        if  isempty(regexpi(thisMarkedImFN,'\.gif$'))
            imMarked = imread([INPUT_DIR f(thisMarkedInd).name]);
            imMarkedResized = imresize(imMarked,scaleVec);
        else % is .gif
            [imMarked,imMarkedMap] = imread([INPUT_DIR f(thisMarkedInd).name]);
